...
export default function MediaComments({ openComments, commentId }) {
  ...
  const sendComment = async (e) => {
    ...
    await addDoc(collection(db, 'posts', commentId, 'comments'), {
  ...
  useEffect(
    () =>
      onSnapshot(
        query(collection(db, 'posts', commentId, 'comments'),
            ...
            {comments.map((comment) => (
              <div id='non-select' key={comment.id} style={{ display: 'flex', flexDirection: 'column', marginBottom: 10 }}>
                ...
                >
                  ...
                  <Moment fromNow style={{ display: 'flex', width: 300, marginRight: 10, color: 'silver' }}>
