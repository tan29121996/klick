...
export default function PostThread({ openModal }) {
  ...
  const upload = async () => {
    ...
    const imageRef = ref(storage, `forumPosts/${docRef.id}/image`);

    uploadString(imageRef, selectedFile, 'data_url').then((snapshot) => {
      getDownloadURL(snapshot.ref).then((downloadUrl) => {
        updateDoc(
          doc(db, 'forumPosts', docRef.id),
          { image: downloadUrl }
        );
      }).catch((error) => {
        console.log(error);
  ...
  return (
    <div style={{ display: 'flex', width: 670, height: 752, justifyContent: 'center', alignItems: 'center' }}>
      ...
      >
        <div style={{ color: 'silver', marginTop: 10 }}>Create a thread</div>
        <div>
          <input
            ref={titleRef}
            type='text'
        ...
        <div>
          <textarea
            ref={contentRef}
            style={{
              width: 560, height: 375, border: '0px',
